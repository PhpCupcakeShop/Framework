...
spl_autoload_register(function ($Name) {
    /*
    $MyClassesFolder = $_SERVER['DOCUMENT_ROOT'].'/www/MyClasses';
    $MyClasses = AutoloadFunctions::getFoldersForAutoloader($MyClassesFolder);
    print_r($MyClasses);
    foreach ($MyClasses as $MyClass) {
    $myName = str_replace('MyApp', '', $Name);
    $myName = str_replace('\\', '/', $myName);  
    $myName = str_replace('//', '/', $myName);  
        echo $myName;
    $myClassFile = $_SERVER['DOCUMENT_ROOT'] . '/www/MyClasses/'. $myName . '.php';       /*linkhere/
        echo $myClassFile;
    if (file_exists($myClassFile)) { include $myClassFile; }
    }
    */


    $destroyName = str_replace('PhpCupcakes', '', $Name);
    $destroyName = str_replace('\\', '/', $destroyName);  
    $CupcakeClassFile = $_SERVER['DOCUMENT_ROOT'] . '/Framework/'. $destroyName . '.php';       /*linkhere*/
    ...
    $modelFile = $_SERVER['DOCUMENT_ROOT'] . '/'. $destroyName . '.php';              /*linkhere*/
    $configFile = $_SERVER['DOCUMENT_ROOT'] . '/'. $destroyName . '.php';       /*linkhere*/
    //$classFile = str_replace('//', '/', $classFile);
    if (file_exists($CupcakeClassFile)) {
        include $CupcakeClassFile;
    } elseif (file_exists($configFile)) {
        include $configFile;
    } else {
        echo 'Class file not found: ' . $CupcakeClassFile;
    }
});
